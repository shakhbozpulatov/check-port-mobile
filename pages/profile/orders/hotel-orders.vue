        ...
        <div class="flex items-end justify-between flex-col">
          ...
          >
            ...
            <div class="gap-2 grid grid-cols-2">
              <UButton
                color="primary"
                class="rounded-[1rem] mb-2 flex items-center justify-center"
        ...
        <div class="overflow-x-auto">
          ...
          >
            ...
            <template #item-id="data">
              <NuxtLinkLocale :to="`/orders/hotel/${data.id}`">
...
const items = computed((): Item[] => {
  return filteredItems.value?.map((item) => {
    return {
      ...
      passengers: computed(() => {
        return item?.data?.passenger.map((el: any) => {
...
const getHotelOrders = async (page: number) => {
  try {
    loading.value = true
    const res = await useApi().$get('/v1/hotel/order', {
      params: {
        sort: '-id',
        page: page,
